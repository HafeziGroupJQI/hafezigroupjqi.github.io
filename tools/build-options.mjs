...
export function parseBuildOptions(args, environment = process.env) {
  ...
  const content =
    readOption(args, "--content") ??
    environment.CONTENT_DIR ??
    (mode === "internal" ? "../vault-private" : "content")
  ...
  const publicContent = readOption(args, "--public-content") ?? "content"
  const consumed = new Set(["--public-content", "--mode", "--content", "--output", "--base-url"])
  const quartzArgs = []
